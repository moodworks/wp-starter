...
(function () {
	const SplitBtn = function (element) {
	...
	SplitBtn.prototype.navigateItems = function (event, direction) {
		event.preventDefault();
		const index = Array.prototype.indexOf.call(this.menuItems, event.target);
	...
	SplitBtn.prototype.checkMenuFocus = function () {
		const menuParent = document.activeElement.closest('.js-split-btn-v2');
	...
	SplitBtn.prototype.animateBg = function (bool) {
		if (bool) {
			// expand bg
			const listInfo = this.itemsList[0].getBoundingClientRect();
			const bgInfo = this.morphBg[0].getBoundingClientRect();

			this.morphBg[0].style.transform = `translateX(${
				listInfo.left - bgInfo.left
			}px) translateY(${listInfo.top - bgInfo.top}px) translateZ(-0.1px)`;
	...
	const splitBtn = document.getElementsByClassName('js-split-btn-v2');
	if (splitBtn.length > 0) {
		const splitBtnArray = [];
		for (let i = 0; i < splitBtn.length; i++) {
			(function (i) {
